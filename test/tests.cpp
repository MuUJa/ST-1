...
TEST(CheckPrime, Squares) {
    ...
    int n = 10;
    for (int i = 0; i < n; i++) {
        uint64_t value = rand() % 1'000'000'000;
...
TEST(CheckPrime, ProductTwoBigPrimes) {
    ...
    EXPECT_FALSE(checkPrime(1'000'003LL * 1'000'019LL));
    EXPECT_FALSE(checkPrime(1'000'019LL * 1'000'019LL));
}
...
TEST(NPrime, FarPrimes) {
    EXPECT_EQ(1'000'003, nPrime(78'498));
    EXPECT_EQ(1'000'019, nPrime(78'499));
}
...
TEST(SymPrimeByDiff, bigPrimes) {
    ...
    // Check right bound
    EXPECT_EQ(1e6 + 3, sumPrime(1e6 + 19) - sumPrime(1e6));
    // Check left bound
    EXPECT_EQ(1e6 + 3, sumPrime(1e6 + 10) - sumPrime(1e6 + 3));
    // Check both bounds
    EXPECT_EQ(1e6 + 3, sumPrime(1e6 + 19) - sumPrime(1e6 + 3));
}
